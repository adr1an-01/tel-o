...
// ── Tema & Accent (persiste no localStorage) ──────────────────────────────
function setTheme(t) {
...
function setAccent(a) {
  ...
  document.querySelectorAll('.accent-swatch').forEach(s => {
    const active = s.dataset.accent === a;
    s.classList.toggle('active', active);
    s.textContent = active ? '✓' : '';
  });
}
function aplicarPreferencias() {
  const t = localStorage.getItem('central_theme')  || 'dark';
  const a = localStorage.getItem('central_accent') || 'indigo';
...
function mostrarApp(usuario) {
  ...
  const cu = document.getElementById('configUsuario');
  if (cu) cu.textContent = usuario;
  setupEventos();
  iniciarNavegacao();
  renderizarLivros();
  carregarDados();
...
async function carregarDados() {
  try {
    ...
    renderizarServos(dadosServos); preencherDropdownServos();
...
function renderizarDashboard() {
  ...
  const anivMes = dadosServos.filter(s => {
    if (!s.aniversario) return false;
    return new Date(s.aniversario + 'T12:00:00').getMonth() === hoje.getMonth();
  }).sort((a, b) => new Date(a.aniversario + 'T12:00:00').getDate() - new Date(b.aniversario + 'T12:00:00').getDate());
  ...
  document.getElementById('dashCards').innerHTML = `
    ...
    <div class="dash-card ${anivHoje.length > 0 ? 'accent' : ''}">
      <div class="dash-card-icon">🎂</div>
  ...
  document.getElementById('dashProximas').innerHTML = proximas.length
    ...
    : '<div class="empty-state" style="padding:16px 0">Nenhuma escala futura.</div>';

  document.getElementById('dashAniversarios').innerHTML = anivMes.length
    ? anivMes.map(s => {
        const d = new Date(s.aniversario + 'T12:00:00');
    ...
    : '<div class="empty-state" style="padding:16px 0">Nenhum aniversário este mês.</div>';
...
function abrirDetalhe(dataStr, escalas, dia) {
    ...
    ? escalas.map(e => `
        <div class="detail-escala">
          ...
          <div style="margin-top:10px">
            <button class="btn btn--danger" style="font-size:12px;padding:6px 12px" onclick="excluirEscala(${e.id},'${esc(e.operador)}')">Remover</button>
          </div>
        </div>`).join('')
    : `<div class="empty-state" style="padding:16px 0">Dia livre.<br>
  ...
  d.style.display = 'block'; d.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
...
async function salvarEscala() {
  ...
  try {
    await post({ acao: 'salvar', tipo: 'escala', data: { data, operador, telefone: servo?.telefone || '', musicas: document.getElementById('esc-musicas').value, obs: document.getElementById('esc-obs').value } });
    fecharModal('modal-escala'); limpar(['esc-data', 'esc-operador', 'esc-musicas', 'esc-obs']);
    toast('Escala salva!', 'ok'); carregarDados();
...
function renderizarServos(lista) {
  ...
  el.innerHTML = lista.map(s => {
    ...
    return `
      <div class="servo-card ${s.ativo ? '' : 'servo-inactive'}">
        <button class="servo-card-del" onclick="excluirServo(${s.id},'${esc(s.nome)}')">✕</button>
        <div class="servo-card-top">
          <div class="servo-card-avatar">${iniciais}</div>
          <div class="servo-card-info">
            <div class="servo-card-name">${esc(s.nome)}</div>
            ${s.funcao ? `<div class="servo-card-funcao">${esc(s.funcao)}</div>` : '<div class="servo-card-funcao" style="color:var(--text-3)">—</div>'}
          </div>
        </div>
        ...
        <div class="servo-card-footer">
          ${ehAnivHoje ? `<span class="servo-badge aniversario">🎉 Aniversário hoje!</span>` : ''}
          ${!s.ativo   ? `<span class="servo-badge inativo">Inativo</span>` : ''}
        </div>
      </div>`;
  }).join('');
}
function filtrarServos() {
  ...
  renderizarServos(dadosServos.filter(s => s.nome.toLowerCase().includes(q) || (s.funcao || '').toLowerCase().includes(q)));
}
async function salvarServo() {
  ...
  try {
    await post({ acao: 'salvar', tipo: 'servo', data: { nome, telefone: document.getElementById('srv-telefone').value, funcao: document.getElementById('srv-funcao').value, email: document.getElementById('srv-email').value, aniversario: document.getElementById('srv-aniversario').value } });
    fecharModal('modal-servo'); limpar(['srv-nome', 'srv-telefone', 'srv-funcao', 'srv-email', 'srv-aniversario']);
    toast('Servo salvo!', 'ok'); carregarDados();
  } catch { toast('Erro.', 'err'); }
}
...
async function abrirLivro(slug, nome, totalCaps) {
  ...
  try { const res = await post({ acao: 'listar_anotacoes_livro', livro: slug }); anotacoesLivro = res; }
  catch { anotacoesLivro = { versiculos: [], capitulos: [] }; }
  try { const res = await post({ acao: 'buscar_anotacao_capitulo', livro: slug, capitulo: 0 }); document.getElementById('capNotaTexto').value = res.texto || ''; }
...
async function abrirCapitulo(cap) {
  ...
  try { const res = await post({ acao: 'buscar_anotacao_capitulo', livro: livroAtual.slug, capitulo: cap }); document.getElementById('verseCapNotaTexto').value = res.texto || ''; }
  ...
  try {
    ...
    renderizarVersiculos(livroData.caps[cap - 1].map((texto, i) => ({ verse: i + 1, text: texto })));
...
// ── Renderizar versículos (toque abre modal) ───────────────────────────────
function renderizarVersiculos(verses) {
  ...
  lista.innerHTML = verses.map(v => {
    ...
    return `
      <div class="verse-item ${temNota ? 'has-nota' : ''}" id="verse-${v.verse}"
           onclick="abrirModalVerso(${v.verse}, ${JSON.stringify(v.text.trim()).replace(/'/g,"&#39;")})">
        <span class="verse-num">${v.verse}</span>
        <div class="verse-body">
          <div class="verse-text">${esc(v.text.trim())}</div>
          ${temNota ? `<div class="verse-nota-badge">📝 Anotado</div>` : ''}
        </div>
      </div>`;
  }).join('');
  // Carregar previews
  anotacoesLivro.versiculos.filter(a => a.capitulo == capAtual).forEach(async a => {
    try {
      ...
      if (badge && res.texto) badge.textContent = '📝 ' + res.texto.substring(0, 60) + (res.texto.length > 60 ? '…' : '');
    } catch {}
  });
}
...
function acaoVerso(tipo) {
  ...
  if (tipo === 'anotar') {
    ...
    const conteudo = `"${texto}" — ${ref} (NVI)`;
    navigator.clipboard?.writeText(conteudo).then(() => toast('Copiado! 📋', 'ok')).catch(() => {
      // fallback
      const ta = document.createElement('textarea'); ta.value = conteudo;
    ...
    const conteudo = `"${texto}" — ${ref} (NVI)`;
    if (navigator.share) {
      navigator.share({ title: ref, text: conteudo }).catch(() => {});
    } else {
      navigator.clipboard?.writeText(conteudo).then(() => toast('Texto copiado para compartilhar 🔗', 'ok'));
    }
  }
}

// ── Versículos na busca (toque também abre modal) ─────────────────────────
function renderizarResultadoBiblia(verses, refLabel) {
  ...
  document.getElementById('bibliaResultado').innerHTML = `
    <div class="bible-result">
      ...
      ${verses.map(v => {
        const nomeDisplay = v.nome || livroAtual?.nome || '';
        return `
          <div class="bible-verse" onclick="abrirModalVersoBusca(${v.verse},'${esc(v.text)}','${esc(nomeDisplay)}',${v.cap},'${v.slug||livroAtual?.slug||''}')">
            <span class="bible-verse-num">
              ${nomeDisplay ? `<span style="font-size:9px;display:block;color:var(--accent)">${esc(nomeDisplay)} ${v.cap}</span>` : ''}
              ${v.verse}
            </span>
...
function abrirModalVersoBusca(verse, texto, nomeL, cap, slug) {
  // Salvar contexto temporário para ações da busca
  livroAtual = livroAtual || { slug, nome: nomeL };
  capAtual   = cap;
  versoAtual = { verse, texto };
...
async function confirmarFavorito() {
  ...
  try {
    await post({ acao: 'salvar_favorito', data: { referencia: favoritoTemp.ref, livro: favoritoTemp.livro, capitulo: favoritoTemp.cap, versiculo: favoritoTemp.vers, texto: favoritoTemp.texto, anotacao: document.getElementById('favoritoAnotacao').value, categoria: document.getElementById('favoritoCategoria').value } });
    fecharModal('modal-favorito'); favoritoTemp = null; toast('Favorito salvo! ⭐', 'ok');
...
async function salvarEstudo() {
  ...
  try {
    ...
    fecharModal('modal-estudo'); limpar(['est-titulo', 'est-versiculo', 'est-texto-base', 'est-conteudo']);
    estudoEditandoId = null; document.getElementById('modalEstudoTitulo').textContent = 'Novo Estudo';
...
function editarEstudo(id) {
  post({ acao: 'listar_estudos' }).then(res => {
    ...
    estudoEditandoId = id; document.getElementById('modalEstudoTitulo').textContent = 'Editar Estudo';
...
// Busca por referência/palavra
const NOMES_SLUG = {}, SLUGS_NOMES = {};
async function iniciarMapaNomes() {
  const biblia = await carregarBibliaJSON();
  Object.entries(biblia).forEach(([slug, livro]) => {
    SLUGS_NOMES[slug] = livro.nome; NOMES_SLUG[livro.nome.toLowerCase()] = slug;
    const abrev = livro.abbr?.toLowerCase(); if (abrev) NOMES_SLUG[abrev] = slug;
  });
}
function parsearRefLocal(ref) {
  const m = ref.trim().match(/^(.+?)\s+(\d+)(?::(\d+)(?:-(\d+))?)?$/i); if (!m) return null;
  const [, livroStr, cap, vers, versEnd] = m;
  const slug = NOMES_SLUG[livroStr.trim().toLowerCase()]; if (!slug) return null;
...
async function buscarPorReferencia() {
  ...
  try {
    const biblia = await carregarBibliaJSON(), livroData = biblia[parsed.slug];
    if (!livroData) { el.innerHTML = '<div class="bible-error">Livro não encontrado.</div>'; return; }
    const cap = livroData.caps[parsed.cap - 1]; if (!cap) { el.innerHTML = '<div class="bible-error">Capítulo não encontrado.</div>'; return; }
    let verses;
    if (parsed.vers) { const fim = parsed.versEnd || parsed.vers; verses = cap.slice(parsed.vers - 1, fim).map((t, i) => ({ verse: parsed.vers + i, text: t, slug: parsed.slug, cap: parsed.cap, nome: livroData.nome })); }
    else verses = cap.map((t, i) => ({ verse: i + 1, text: t, slug: parsed.slug, cap: parsed.cap, nome: livroData.nome }));
