...
function setupEventos() {
  ...
  document.getElementById('esc-operador').addEventListener('change', function(){ const s=dadosServos.find(s=>s.nome===this.value); /* telefone só para uso interno, não exibir no form */ });
...
async function ciclarStatus(id,statusAtual){ try{await post({acao:'atualizar',tipo:'tarefa_status',id,status:CICLO[statusAtual]||'pendente'});carregarDados();}catch{toast('Erro.','err');} }
...
async function salvarEscala(){
  ...
  try{ await post({acao:'salvar',tipo:'escala',data:{data,operador,telefone:servo?.telefone||'',musicas:document.getElementById('esc-musicas').value,obs:document.getElementById('esc-obs').value}}); fecharModal('modal-escala'); limpar(['esc-data','esc-operador','esc-musicas','esc-obs']); toast('Escala salva!','ok'); carregarDados(); }
  catch{ toast('Erro ao salvar.','err'); }
}
async function salvarTarefa(){ const tarefa=document.getElementById('tar-tarefa').value.trim(); if(!tarefa){toast('Descrição é obrigatória.','err');return;} try{await post({acao:'salvar',tipo:'tarefa',data:{tarefa,prazo:document.getElementById('tar-prazo').value,status:document.getElementById('tar-status').value}});fecharModal('modal-tarefa');limpar(['tar-tarefa','tar-prazo']);toast('Tarefa salva!','ok');carregarDados();}catch{toast('Erro.','err');} }
  ...
  const render = (lista, elId) => {
    ...
    el.innerHTML = lista.map(l => `
      <button class="book-btn" onclick="abrirLivro('${l.slug}','${esc(l.nome)}',${l.caps})">
        ...
        <span class="book-abbr">${l.caps} caps</span>
      </button>`).join('');
  };
...
function abrirAnotacaoVerso(verso, texto) {
  ...
  document.getElementById('anotacaoVersoPreview').textContent = `${livroAtual.nome} ${capAtual}:${verso} — "${texto.substring(0,100)}${texto.length>100?'…':''}"`;
...
async function salvarAnotacaoVerso() {
  ...
  try {
    ...
    fecharModal('modal-anotacao-verso');
    toast('Anotação salva!','ok');
    // Atualizar anotacoesLivro e re-renderizar
...
async function confirmarFavorito() {
  ...
  try {
    const res=await post({acao:'salvar_favorito',data:{referencia:favoritoTemp.ref,livro:favoritoTemp.livro,capitulo:favoritoTemp.cap,versiculo:favoritoTemp.vers,texto:favoritoTemp.texto,anotacao:document.getElementById('favoritoAnotacao').value,categoria:document.getElementById('favoritoCategoria').value}});
...
async function excluirFavorito(id){ if(!confirm('Remover?'))return; try{await post({acao:'excluir_favorito',id});toast('Removido.','ok');carregarFavoritos();}catch{toast('Erro.','err');} }
